...
def _build_proactive_json_instruction(
    ...
    monologue = (
        ...
        f"**不要凭猜测就说对方回来了。** 你此刻看不到{user_name}在不在，"
        f"只能靠场景画面（过道/门/家具/光线）或时间推测--那只是「猜」。"
        f"猜的时候**不要**直接说「欢迎回来/おかえり」这类肯定对方已在的话；"
        f"要么温和地问一句确认（如「回来了吗？」），要么选 silent 只在心里想。"
        f"只有明确看到{user_name}本人（主动 look 看到、或被动画面明确拍到人）才能肯定地打招呼。\n\n"
...
def _build_proactive_cot_instruction(
    ...
    monologue = (
        ...
        f"**不要凭猜测就说对方回来了。** 你此刻看不到用户在不在，"
        f"只能靠场景画面（过道/门/家具/光线）或时间推测--那只是「猜」。"
        f"猜的时候**不要**直接说「欢迎回来/おかえり」这类肯定对方已在的话；"
        f"要么温和地问一句确认（如「回来了吗？」），要么用 `[决定：沉默]` 只在心里想。"
        f"只有明确看到用户本人（主动 look 看到、或被动画面明确拍到人）才能肯定地打招呼。\n\n"
